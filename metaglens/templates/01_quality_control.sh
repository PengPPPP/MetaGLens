...
#SBATCH --mem={{MEMORY}}
#SBATCH --output={{WORK_DIR}}/metaglens_results/reports/logs/01_qc_%j.log
...
# Read a total read count out of a fastp JSON report instead of decompressing
# the FASTQ files again. fastp counts both mates, so the value already covers
# R1+R2. Prints -1 when the value cannot be read, so callers can fall back.
fastp_read_count() {
    local json="$1"
    local phase="$2"   # before_filtering | after_filtering
    if [[ ! -r "${json}" ]]; then
        printf '%d' -1
        return 0
    fi
...
try:
    with open(sys.argv[1], encoding="utf-8") as fh:
        print(int(json.load(fh)["summary"][sys.argv[2]]["total_reads"]))
except Exception:
    print(-1)
' "${json}" "${phase}"
}
...
process_qc_sample() {
    ...
    local FASTP_JSON="${OUTPUT_DIR}/${SAMPLE}_fastp.json"
    ...
    # Reuse the counts fastp already computed rather than re-reading the raw FASTQs.
    BEFORE_TOTAL=$(fastp_read_count "${FASTP_JSON}" before_filtering)
    if [[ "${BEFORE_TOTAL}" -lt 0 ]]; then
        log "  [${SAMPLE}] WARNING: fastp JSON unreadable; counting raw reads directly."
        BEFORE_TOTAL=$(( $(count_fastq_reads "${R1}") + $(count_fastq_reads "${R2}") ))
    fi
    log "  [${SAMPLE}] raw reads (R1+R2): ${BEFORE_TOTAL}"
    ...
    # Host/PhiX depletion runs after fastp, so fastp's "after" count is only
    # authoritative when neither ran; otherwise count the final clean files.
    if [[ "${REMOVE_HOST}" == "yes" || "${REMOVE_PHIX}" == "yes" ]]; then
        AFTER_TOTAL=$(( $(count_fastq_reads "${CLEAN_R1}") + $(count_fastq_reads "${CLEAN_R2}") ))
    else
        AFTER_TOTAL=$(fastp_read_count "${FASTP_JSON}" after_filtering)
        if [[ "${AFTER_TOTAL}" -lt 0 ]]; then
            AFTER_TOTAL=$(( $(count_fastq_reads "${CLEAN_R1}") + $(count_fastq_reads "${CLEAN_R2}") ))
        fi
    fi
    log "  [${SAMPLE}] clean reads (R1+R2): ${AFTER_TOTAL}"
