...
## Phase 18 — 黑客松演示站(外滩黑客松·AI Coding 大赛)

**背景与硬约束**
- 赛事:外滩黑客松·AI Coding 大赛,线上 **7/20 – 8/9**(今天 7/31,**剩约 9 天**)。
  九大平台承办(含 Qoder、WeaveFox)。主题「全民 Coding」,验证"AI 能否把想法变成数字产品"。
- **根本错配(必须正面解决)**:MetaGLens 是 CLI + 4000 行 bash + 18 个 conda 工具 +
  200 GB 数据库 + 数小时运行。**评委不可能装环境、下数据库、等几小时**。
  直接提交现形态 = 没人能验证它能跑。
- **解法**:不改造软件本体,做一个**网页演示壳**,把已有能力在浏览器里 1 分钟内讲完。

**已有的可复用资产(这是能 9 天做成的前提)**
| 资产 | 价值 |
|---|---|
| 桩工具 demo,**单路由 9.2 秒**跑完全流程 | 可做成网页里点一下就跑完的**真实**演示 |
| 配置页(自包含、中英双语、实时探测) | 评委能亲手操作 |
| `report.html` / `monitor.html`(各约 300 KB 自包含) | 现成视觉成品 |
| `_theme.py` 共享皮肤 | 风格统一,无需重做设计 |

**平台中立原则(重要)**:参赛平台未最终确定,故演示站必须是**自包含、可部署到任意
能跑 Python 3.8+ 的环境**,不绑定任何平台特有 SDK;同时**保留纯静态导出能力**作为兜底
(万一只能传静态文件)。

**诚实性红线(不可协商)**
- 页面必须**显著标注**演示用桩工具、不产生科学结果;
- **绝不**伪造科学数据或声称跑了真实分析;
- 演示跑的是**真实的** 12 阶段控制流、状态机、产物验证与报告生成器,只把耗时的生信
  工具换成桩——这一点要讲清楚,它恰好证明"生成的脚本可独立运行"这一核心卖点。

### 18.A 演示站骨架(`metaglens/showcase/`)

- [ ] 18.A1 新建 `metaglens/showcase/`,基于 **stdlib `http.server`**(与 webconfig 同构,
  不引 Flask/FastAPI,保持零依赖)。入口 `metaglens showcase [--port N] [--host H]`。
  **注意**:演示站面向公网评委,与 webconfig 的 loopback+token 定位不同——
  需支持绑 `0.0.0.0`,但**必须只暴露只读演示接口**,不得暴露任何写用户文件/执行任意命令的能力。
- [ ] 18.A2 一页式动线(单页,锚点分节),复用 `_theme.py`:
  ① 项目是什么 + 为什么存在(真实痛点:实验室服务器不许装计费 agent)
  ② **亲手配置**(嵌入配置页表单,预填演示值)
  ③ **点击运行** → 后端跑桩 demo → 前端轮询/自刷新展示 `monitor.html` 式进度
  ④ **看报告** → 展示真实 `report.html`
  ⑤ **读生成的脚本** → 展示某个阶段的真实 bash(证明产物可独立运行、可审计)
- [ ] 18.A3 并发与安全:演示跑放**队列 + 单并发 + 超时**(桩 9 秒,给 60 秒上限),
  每次跑在独立临时目录、跑完清理;**限制同时排队数**,防止评委同时点击打爆机器。
- [ ] 18.A4 纯静态兜底:`metaglens showcase --export DIR` 导出一份**预跑好**的静态站
  (含已生成的 report/monitor/脚本片段),无后端也能看完整故事。

### 18.B 叙事与素材(得分关键,不只是技术)

- [ ] 18.B1 站内文案讲清三层(中英双语,复用 i18n):
  ① **真实痛点**——共享服务器不许装计费型 agent,所以需要确定性、零密钥、零外呼的软件;
  ② **AI Coding 历程**——9 个 skill → 确定性 CLI;
  ③ **差异化亮点:两个 AI agent 协作开发 + 互相独立审查**,全程留 `WORKLOG.md` 与 git
     审计轨迹(如:一方发现 nullglob 字面量数组 bug、另一方用"注回 bug"验证 demo 有牙齿、
     以及对 repair 白名单的 7 种攻击测试)。**这是别人交不出来的东西。**
- [ ] 18.B2 站内放一个「**开发审计轨迹**」小节:引用 git 提交数、测试数(248)、
  以及 WORKLOG 里几个真实的"互相抓错"案例。**只用真实数据,不夸大。**
- [ ] 18.B3 准备小红书素材(由用户本人发布):3–5 张截图 + 一段 GIF(配置→运行→报告)
  + 一段文案草稿。素材放 `preview/` 或 `docs/assets/`(不入库或按需入库)。

### 18.C 门禁与提交物

- [ ] 18.C1 测试:showcase 的路由/队列/超时/清理逻辑;`--export` 静态站可打开;
  **安全断言**:演示接口不接受任意路径写入、不执行任意命令、不泄露服务器路径。
- [ ] 18.C2 全量门禁照旧(裸解释器 unittest + bash -n + demo + compileall)。
- [ ] 18.C3 `README` 增「在线演示」小节 + 部署说明(一条命令起站)。
- [ ] 18.C4 commits:`feat(showcase): hackathon demo site`、`docs: showcase & deployment`。

**待用户确认(不阻塞开工)**:最终参赛平台(Qoder 赛道 or WeaveFox);
若平台对部署形式有特殊要求,再按需适配——因设计为平台中立,适配成本低。
